// Board A: I2C1 master — encrypted text message sender
// SCL: D15 (PTE1), SDA: D14 (PTE0), ALT6
// Encrypts messages with substitution cipher before sending to slave 0x48
// Packet format: [0xAA][len][encrypted ascii bytes...]
// Compile with: BOARD_A_ENCRYPTED

#ifdef BOARD_A_ENCRYPTED
...
static void encrypt(const char *plain, char *out, uint8_t len)
{
    for (uint8_t i = 0u; i < len; i++) {
        char c = plain[i];
        if (c >= 'A' && c <= 'Z') {
            out[i] = (char)(cipher_key[c - 'A'] - ('a' - 'A'));
        } else if (c >= 'a' && c <= 'z') {
            out[i] = cipher_key[c - 'a'];
        } else {
            out[i] = c;
        }
    }
...
int main(void)
{
    ...
    for (;;) {
        for (uint8_t r = 0u; r < SENDS_PER_MESSAGE; r++) {
            encrypt(messages[idx], enc_buf, lengths[idx]);
            send_text(enc_buf, lengths[idx]);
            delay_ms(250u);
        }
